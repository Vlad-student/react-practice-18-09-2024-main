...
const Quiz = () => {
    ...
    const showOptions =(item) => <option key={item} value={item} >{item}</option>
    const changeAnswer3 = (event) => {setAnswer3(event.target.value)}
    const showRadio = (item) => <label key={item}>
        <input type='radio' name='question1' value={item} 
        checked={item === answer1} onChange={changeAnswer1} /> 
        {item}
        </label> 
...
const newArray = answer2.includes(value) ? answer2.filter((v)=>v!==value) :[...answer2,value]  ;
        ...
        const showCheckbox = (item) => <label key={item}>
            <input type="checkbox" value={item} 
            checked={answer2.includes(item)} onChange={changeAnswer2} />
            {item}
        </label>
        const checkResult = () => {
let result =0;
if (answer1===correctAnswers.answ1) {result++}
if (answer2===correctAnswers.answ2) {result++}
if (answer2.length === correctAnswers.answ2.length && answer2.every((item)=>
     correctAnswers.answ2.includes(item))) {result++}
setScore(result)
        }
    return (
        <div>
            <fieldset>
                <legend>1. Яка бібліотека використовується для створення інтерфейсів ?
                     </legend>
                {
            ...
            <fieldset>
                <legend>2. Якими мовами можна змінити стилі елементів веб-сторінок ? </legend>
